#!/bin/ksh
######################################################################
# Script :      clickstream_reference_table_load.ksh 
#
#######################################################################
...
. $SCRIPTS_DIR/load_datastage_params.ksh
   ...
   export JOB_RUN_ID=`perl createJobRun.pl "seq_clickstream_reference_prsnt_load" "$SCRIPT_VERSION"`

   print_msg "0002 Current Job ID = $JOB_RUN_ID"  


print_msg "0010 unprcoessed job run ids"
	set -A predjobids `perl findUnprocessedJobRunsFlow.pl "seq_clickstream_reference_prsnt_load"`
...
print_msg "0213 Process started for the presentation load of granular clickstream"
    		...
    		dsjob -run -param job_run_id="$JOB_RUN_ID" -param pjob_run_id="$PRED_JOBS_IDS" -wait -warn 0 -jobstatus $DATA_STAGE_PROJECT seq_clickstream_reference_prsnt_load
