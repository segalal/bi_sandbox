...
. $SCRIPTS_DIR/load_source_table_functions.ksh 
...
#debug $debug_ind "SOURCE DATA values are:" "SOURCE_DATA_KEY=$SOURCE_DATA_KEY" "ENCRYPTION_FLAG=$ENCRYPTION_FLAG" 
...
then
  ...
  fnLoadFromTable $feed_target_table $feed_source_name $SOURCE_DATA_KEY $ENCRYPTION_FLAG $etl_run_id $debug_ind
elif [[ $feed_source_format = "STANDARD_INPUT" ]]
then
  echo ""
  print_msg "processing started for source type: standard input"
  fnLoadFromStdInput $feed_target_table $SOURCE_DATA_KEY $feed_source_name $ENCRYPTION_FLAG $etl_run_id $debug_ind $lineno_delimiter $ignore_zero $max_error_number
else
  echo ""
  print_msg "processing started for source type: file"
  fnLoadFromFile  $feed_target_table $feed_area $feed_source_name $feed_file_name_zipped $feed_file_zip_mode $feed_file_delim $feed_file_zipped_ind $feed_catchup_location $feed_file_remove_ind $SOURCE_DATA_KEY $ENCRYPTION_FLAG $etl_run_id $debug_ind $multiple_feed_files_ind $missing_feed_files_permitted_ind
fi
